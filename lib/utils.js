...
// plan selection
const basicPlans = ctx => {
	let message = "Select one of the following to proceed. \n"; 
...
// update for the year 2025
const newUpdate = async (ctx, msge) => {
	let message =  msge || "Select one of the following to proceed. \n"; 

	let markup = Markup.inlineKeyboard([
			[Markup.button.callback("Monthly @ KES: 1500", "subscribe_monthly")],
			[Markup.button.callback("Annually @ KES: 15000", "subscribe_annually")],
		]).oneTime();

	return await ctx.reply(message, markup);
}
const plans = async (ctx) => {
	try {
		...
		// let markup = Markup.inlineKeyboard([
		// 	[Markup.button.callback("Basic Classes", "basic")],
		// 	[Markup.button.callback("Premium Classes", "premium")],
		// ]).oneTime();
		return await newUpdate(ctx, message)
		...
		if (err.response && err.response.error_code === 403) {
			console.log(`User ${ctx.from.id} - ${ctx.from.first_name} has blocked the bot.`);
		  } else {
			console.log(`Failed to send message to ${ctx.from.id}: ${err.message}`);
		  }
