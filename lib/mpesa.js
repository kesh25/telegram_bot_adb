...
class MPESA {
  ...
  generateToken() {
    ...
    let token = this.makeAxiosRequest("get", null, headers, url);
	console.log(token)
  ...
  async STKPush() {
    ...
    try {
      ...
      let res = this.makeAxiosRequest("post", payment, null, url);
  ...
  makeAxiosRequest(method, data, headersObject, url) {
    let headers = headersObject || {
      Authorization: "Bearer " + this.token,
    };

	let response = unirest(method.toUpperCase(), url)
      .headers(headers)
      .send(data)
      .end((res) => {
		console.log("error ---->", res.body || res.error)
        if (res.error) {
          return {
            status: "fail",
            code: res.error.response?.status || 400,
            data: res.error.response?.data || res.error,
          };
        } else {
          return {
            status: "success",
