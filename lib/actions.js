...
const handlePaymentPrompt = async (type, ctx, userState) => {
	...
	let mpesa = new MPESA(phoneNumber, userState.amount);
...
const handleRetryMpesa = async (ctx, userState) => {
	...
	// initiate mpesa payment prompt here
	 amount = subscription.payment_details.amount; 
	// get mpesa - CheckoutRequestID and update subscription doc with it
...
const handleCheckMpesaStatus = (ctx, userState) => {
	console.log(userState, "status");
	console.log();
	console.log(ctx);
	// get userId to fetch mpesa ref from db

	// use the ref to check status

	// if status is success -> send message to user

	// if status is failed -> send message to user
};
