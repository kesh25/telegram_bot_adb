...
const handleRetryMpesa = async (ctx, userState) => {
	...
	let confirm = await Subscription.findOne({user_id: userId, status: "active"}); 
	if (confirm) {
...
const handleConfirm = async (ctx, userState) => {
	...
	let confirm = await botInstance.bot.telegram.getChatMember(process.env.CHANNEL_ID, user.user_id); 

	if (["creator", "administrator", "member"].includes(confirm.status)) {
		ctx.reply("You are already added to the channel"); 
	} else {
		ctx.reply(`Remember you should have a request to join the channel through the link: \n ${process.env.INVITE_LINK}`)
