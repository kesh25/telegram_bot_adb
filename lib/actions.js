...
const handleConfirm = async (ctx) => {
  ...
  if (!subscription) {
    let checkIfUserInChannel = await botInstance.verifyJoin(userState.userId);
    if (checkIfUserInChannel) await botInstance.removeUserFromChannel(userState.userId)
    ctx.reply("You have no active subscription.");
    return plans(ctx);
  }

  // check if user is member
  let confirm = await botInstance.verifyJoin(user.user_id);
...
const handleCheckMpesaStatus = async (ctx) => {
  ...
  if (user) {
    ...
    if (status === "active") {
      await botInstance.addUserToChannel(user.user_id);
    }
    ctx.reply(message);
  }
};
...
const handleVerifyJoin = async ctx => {
  ...
  let checkIfUserInChannel = await botInstance.verifyJoin(user_id); 
  ...
  let add = await botInstance.addUserToChannel(user_id); 

  if (add) {
    return ctx.reply("You were added to the channel")
  } else {
    return ctx.reply(`Click on the invite link -> ${process.env.INVITE_LINK} first before tapping the verify membership button.`,
      Markup.inlineKeyboard([
        Markup.button.callback("Verify membership", "verify_join")
      ])
    )
